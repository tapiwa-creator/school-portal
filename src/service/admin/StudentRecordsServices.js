// AdminAcademicRecords.jsx
// Firebase-integrated Academic Records management for admins.
// Reads students from Firestore (AdminStudentService) and writes academic
// data back via updateStudent().  No mock data.

import { useState, useEffect } from "react";
import AdminStudentService from './StudentManagementService'
...
// Default subjects – admin can extend later
const DEFAULT_SUBJECTS = ["Mathematics", "English", "Science", "Social Studies", "Art"];

// ── Small UI atoms ─────────────────────────────────────────────────────────
const ProgressBar = ({ value, max = 100, color = "bg-green-500", height = "h-2" }) => (
    <div className={`w-full ${height} bg-gray-100 rounded-full overflow-hidden`}>
        <div className={`${height} ${color} rounded-full transition-all duration-500`} style={{ width: `${Math.min((value / max) * 100, 100)}%` }} />
    </div>
);
...
function exportCSV(students, term) {
    ...
    students.forEach(s => {
        const recs = s.academic?.[key] || [];
        recs.forEach(r => rows.push([s.name, s.id, s.grade, r.subject, r.test1, r.test2 || "", r.assign || "", r.exam, r.avg, r.letter]));
    });
...
function AddRecordPanel({ students, onClose, onSave }) {
    ...
    const emptyMarks = () => DEFAULT_SUBJECTS.reduce((acc, s) => ({
        ...acc, [s]: { test1: "", test2: "", assign: "", exam: "" }
    }), {});
    ...
    const handleSave = async () => {
        const records = DEFAULT_SUBJECTS
            .filter(s => marks[s].test1 !== "" && marks[s].exam !== "")
            .map(s => {
                const { test1, test2, assign, exam } = marks[s];
                return makeRecord(s, Number(test1), Number(test2) || 0, assign !== "" ? Number(assign) : null, Number(exam));
            });
        ...
        <div className="fixed inset-0 z-[9999] flex" onClick={onClose}>
            ...
            <div className="h-full w-full max-w-[520px] bg-white flex flex-col"
                ...
                <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
                    <div>
                        <h2 className="text-lg font-bold text-gray-900">Add / Edit Academic Records</h2>
                    ...
                    {DEFAULT_SUBJECTS.map(subject => (
                        <div key={subject} className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                            ...
                            <div className="grid grid-cols-4 gap-2">
                                {[{ label: "Test 1", key: "test1" }, { label: "Test 2", key: "test2" }, { label: "Assign", key: "assign" }, { label: "Exam", key: "exam" }].map(({ label, key }) => (
                                    <div key={key}>
                ...
                <div className="px-6 py-5 border-t border-gray-100 flex gap-3 flex-shrink-0">
                    <button onClick={onClose} className="flex-1 py-3 rounded-xl text-sm font-semibold border border-gray-200 bg-white hover:bg-gray-50 transition-colors text-gray-600" disabled={saving}>Cancel</button>
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center"
            ...
            <div className="bg-white rounded-2xl w-[380px] max-w-[95%] text-center"
                ...
                <p className="text-sm text-gray-400 mb-6 leading-relaxed">
                    This will make {term} results visible to <span className="font-semibold text-gray-700">{count} students</span> and their parents.
                </p>
                <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 py-3 rounded-xl text-sm font-semibold border border-gray-200 bg-white hover:bg-gray-50 transition-colors text-gray-600" disabled={publishing}>Cancel</button>
                    ...
                    <div className="flex items-start justify-between mb-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-green-300 text-xs mt-0.5">{student.grade} · {term}</p>
                            </div>
                        </div>
                                    ...
                                    {records.length === 0
                                        ...
                                        : records.map((r, i) => (
                                            <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-4 py-3"><span className={`text-xs font-bold px-2 py-0.5 rounded-full ${r.color}`}>{r.letter}</span></td>
...
export default function AdminAcademicRecords() {
    ...
    const pubKey = termKey; // published uses same keys: term1/term2/term3
    ...
    // ── Save records (Firestore write) ────────────────────────────────────────
    const handleSaveRecords = async ({ studentId, term: recTerm, records }) => {
        const key = TERM_KEY[recTerm];
        const path = `academic.${key}`;
        const result = await AdminStudentService.updateStudent(studentId, { [path]: records });
        if (result.success) {
            // Optimistic local update
            setStudents(ss => ss.map(s =>
                s.id !== studentId ? s : {
                    ...s,
                    academic: { ...s.academic, [key]: records },
                }
            ));
    ...
    return (
        <div className="min-h-screen bg-[#f0f4f0] p-3 md:p-6 space-y-5 font-sans pt-16 md:pt-6">
                                            ...
                                            <td className="px-5 py-3.5">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="text-xs text-gray-400">{s.id || s.docId}</div>
                                                    </div>
                                                </div>
                                            </td>
            ...
            {/* Panels & Modals */}
            {showAdd && <AddRecordPanel students={students} onClose={() => setShowAdd(false)} onSave={handleSaveRecords} />}
