...
const StudentsIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>;
...
const EventIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>;
...
const NAV_MAIN = [
  ...
  { label: "Students", path: "/admin/students", Icon: StudentsIcon },
  { label: "Academic Records", path: "/admin/academic-records", Icon: AcademicRecordsIcon, badge: 5 },
  { label: "Timetable", path: "/admin/timetable", Icon: TimetableIcon },
  { label: "Assignments", path: "/admin/assignments", Icon: AssignIcon, badge: 14 },
  ...
  { label: "Profile", path: "/admin/profile", Icon: ProfileIcon }, // Profile module added
];
...
function SidebarContent({ pathname, onNavigate, onLogout }) {
  return (
    <>
      {/* Brand block */}
      <div className="flex items-center gap-3 px-5 py-[22px]" style={{ borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden"
          style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.15)" }}>
          <img src="/image.png" alt="CS" className="w-full h-full object-cover"
            onError={e => { e.target.style.display = "none"; e.target.parentNode.innerHTML = `<span style="color:#fff;font-weight:800;font-size:13px">CS</span>`; }} />
        </div>
        <div>
          <p className="text-white font-bold text-[13.5px] leading-tight tracking-wide">CORNER STONE</p>
          <p className="text-[10px] font-semibold tracking-[0.1em] uppercase mt-0.5" style={{ color: "#c8a84b" }}>
            Admin Portal
          </p>
        </div>
      </div>
      ...
      <div className="px-5 pt-5 pb-2">
        <span className="text-[10px] font-semibold tracking-[0.12em] uppercase text-white/30">Main Menu</span>
      </div>
      ...
      {/* Logout */}
      <div className="pt-2 pb-3" style={{ borderTop: "1px solid rgba(255,255,255,0.08)" }}>
        <button
          onClick={onLogout}
...
export default function AdminSidebar() {
  ...
  const handleLogout = () => { setMobileOpen(false); setShowConfirm(true); };
  const handleMobileNav = () => setMobileOpen(false);

  return (
    <>
      {/* ── MOBILE TOP BAR ── */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-[9998] flex items-center justify-between px-4 h-14"
        style={{ background: "#0d2818", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden"
            style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.15)" }}>
            <img src="/image.png" alt="CS" className="w-full h-full object-cover"
              onError={e => { e.target.style.display = "none"; e.target.parentNode.innerHTML = `<span style="color:#fff;font-weight:800;font-size:12px">CS</span>`; }} />
          </div>
          <div>
            <p className="text-white font-bold text-[12.5px] leading-tight tracking-wide">CORNER STONE</p>
            <p className="text-[9px] font-semibold tracking-[0.1em] uppercase" style={{ color: "#c8a84b" }}>Admin Portal</p>
          </div>
        </div>
        <button onClick={() => setMobileOpen(true)}
          className="flex flex-col justify-center items-center w-9 h-9 rounded-lg gap-[5px] bg-transparent border-none cursor-pointer hover:bg-white/[0.08] transition-colors" aria-label="Open menu">
      ...
      {/* ── MOBILE DRAWER ── */}
      {mobileOpen && (
        <div onClick={() => setMobileOpen(false)} className="md:hidden fixed inset-0 z-[9999]"
          style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(2px)" }}>
          <aside onClick={e => e.stopPropagation()} className="flex flex-col h-full"
            style={{ width: 240, background: "#0d2818", animation: "slideIn 0.22s ease" }}>
            <SidebarContent pathname={pathname} onNavigate={handleMobileNav} onLogout={handleLogout} />
          </aside>
        </div>
      )}

      {/* ── DESKTOP SIDEBAR ── */}
      <aside className="hidden md:flex flex-col flex-shrink-0 h-screen" style={{ width: 240, background: "#0d2818" }}>
        <SidebarContent pathname={pathname} onNavigate={undefined} onLogout={handleLogout} />
      </aside>
        ...
        <div onClick={() => setShowConfirm(false)} className="fixed inset-0 z-[99999] flex items-center justify-center"
          ...
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl text-center w-[360px] max-w-[90%]"
            ...
            <p className="text-sm text-gray-400 mb-8 leading-relaxed">
              Are you sure you want to log out of the Admin Portal? Any unsaved changes will be lost.
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={() => { setShowConfirm(false); navigate("/"); }}
